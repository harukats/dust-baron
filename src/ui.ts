...
export function label(scene: Phaser.Scene, x: number, y: number, str: string, size: number,
  color: number = COLOR.text, originX = 0, originY = 0): Phaser.GameObjects.Text {
  return scene.add
    .text(x, y, str, {
      fontFamily: FONT, fontSize: `${size}px`, fontStyle: 'bold', color: css(color),
      stroke: css(COLOR.shadow), strokeThickness: Math.max(3, Math.round(size / 7)),
      resolution: 2,
    })
...
export function drawPlate(g: Phaser.GameObjects.Graphics, x: number, y: number, w: number, h: number,
  fill: number = COLOR.rustDark, rim: number = COLOR.chromeDim, alpha = 0.92, rimW = 3): void {
  ...
  for (const [px, py] of [[x + o, y + o], [x + w - o, y + o], [x + o, y + h - o], [x + w - o, y + h - o]] as const) {
...
export function plateButton(scene: Phaser.Scene, x: number, y: number, w: number, h: number, text: string,
  size: number, onClick: () => void, opts: { fill?: number; rim?: number; color?: number } = {}):
  { g: Phaser.GameObjects.Graphics; text: Phaser.GameObjects.Text; zone: Phaser.GameObjects.Zone; redraw: (rim: number) => void } {
  const g = scene.add.graphics();
  const redraw = (rim: number): void => { g.clear(); drawPlate(g, x, y, w, h, opts.fill ?? COLOR.rustMid, rim, 1, 3); };
